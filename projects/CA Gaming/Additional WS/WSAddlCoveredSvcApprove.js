...
module.exports.main = function (ffCollection, vvClient, response) {
  /*Script Name:   AddlCoveredSvcApprove
      ...
      06/10/2014 - Austin: Update to prepare for production usage.
      ...
      08/11/2014 - Jason: Issue with getting the right client intake forms when they are entered in the new fiscal year but pertain to previous year.  Change client
                         intake query to use intake date and not modify date.  Added a longer period to insure intakes for period are brought in.
  ...
  invoiceablePrograms[2] = "Problem Gambling Telephone Intervention"; //Added this line on 04/24/2020
  ...
  logger.info("Enter into process AddlCoveredSvcApprove.");

  //Get the key fields from the provider form to utilize during the process.
  var formProviderId = ffCollection.getFormFieldByName("Provider ID");
  var formCoveredDate = ffCollection.getFormFieldByName("Covered Date");

  var whoIsProvider = formProviderId.value; //Used to know who is provider after information gathered.
  var whoIsProvName = "";
  var whoIsProvRev = "";
  ...
  /*
    if (thisMonth > 6) {
        beginFiscalDate = '7/1/' + thisYear;
        endFiscalDate = '6/30/' + (thisYear + 1);
    } else {
        beginFiscalDate = '7/1/' + (thisYear - 1);
        endFiscalDate = '6/30/' + thisYear;
    }
    */
  ...
  //Provider to Update
  var formProviderData = {}; //Setup an object to query and get exemptions for this case, client and provider id.
  formProviderData.q = "[ProviderID] eq '" + formProviderId.value + "'";
  formProviderData.fields = "InstanceName,createdate,createby,ProviderID";
  formProviderData.offset = 0;
  formProviderData.limit = 1000;
  ...
  for (var item in invoiceablePrograms) {
    intakeQuery =
      intakeQuery + "[Type of Program] eq '" + invoiceablePrograms[x] + "' OR ";
    x = x + 1;
  }
  ...
  Q.allSettled([
    vvClient.forms.getForms(formQuestData, formIDTreatmentQuestionnaire), //Nodejs helper function to get the list of questionnaires with same info as current.
    vvClient.forms.getForms(formAddlSvcData, formIDAddlCoveredServices), //Nodejs helper function to get the list of additional covered services forms.
    vvClient.forms.getForms(formProviderData, formIDProviderForm), //Nodejs helper function to get the list of invoices.
    ...
    var promiseProviderForm = promises[2]; //Needed to get the provider form so the notification checkbox can be updated.
    var promiseClientIntake = promises[3];
    var promiseGroupScreen = promises[4];
    var promiseGroupIntreat = promises[5];
    var promiseGroupEOT = promises[6];
    var promiseSupervisorLog = promises[7];

    try {
      //Gets the site for selected provider to update notification.
      if (promiseProviderForm.state == "fulfilled") {
        //Clean results from the getForms call.
        var formProviderData = JSON.parse(promiseProviderForm.value);
        if (
          ...
          if (typeof formProviderData.data[0].providerID != "undefined") {
            whoIsProvRev = formProviderData.data[0].revisionId; //Get the form revision id for update of notification checkbox.
          }
        } else {
          logger.info("Selected Provider form not found");
          outputCollection[0] =
            "A form was not found for provider " + whoIsProvider + ".";
      ...
      //Get all questionnaires for a client to help calculate PO Usage.
      if (promiseQuestionnaires.state == "fulfilled") {
        ...
        if (
          ...
          for (var a = 0; a < formQuestResultData.data.length; a++) {
            ...
            if (findResults != -1) {
              if (
                formQuestResultData.data[a]["questionnaire Type"] == "Intake"
              ) {
          ...
          outputCollection[0] =
            ...
            whoIsProvName +
            ".  Questionnaires need to exist to request a PO Limit.";
          outputCollection.push();
          response.json(200, outputCollection);
          return false;
        }
      ...
      //Get all ACS forms for a provider to calculate PO Limit.
      ...
      if (promiseGroupScreen.state == "fulfilled") {
        //Clean results from the getForms call.
        var formGroupScreenData = JSON.parse(promiseGroupScreen.value);
        if (
          ...
          if (groupScreeningBillable == 1) {
            ...
            for (var a = 0; a < formGroupScreenData.data.length; a++) {
      ...
      if (promiseGroupIntreat.state == "fulfilled") {
        //Clean results from the getForms call.
        var formGroupIntreatData = JSON.parse(promiseGroupIntreat.value);
        if (
      ...
      if (promiseSupervisorLog.state == "fulfilled") {
        //Clean results from the getForms call.
        var formSupervisorData = JSON.parse(promiseSupervisorLog.value);
        if (
      ...
      if (typeof formACSData.data != "undefined") {
        for (var i = 0; i < formACSData.data.length; i++) {
          if (
            typeof formACSData.data[i]["pO Increment"] != "undefined" &&
            typeof formACSData.data[i]["request Status"] != "undefined"
          ) {
            if (formACSData.data[i]["request Status"] == "Approved") {
              if (formACSData.data[i]["pO Increment"] == null) {
                ...
                poLimit = poLimit + formACSData.data[i]["pO Increment"];
              }
              numApprovedACS++;
            ...
            if (formACSData.data[i]["request Status"] == "Approved") {
              poLimit = poLimit + basePOValue; //This takes care of old legacy forms.
              numApprovedACS++;
            } else if (formACSData.data[i]["request Status"] == "Denied") {
              numDeniedACS++;
            }
      ...
      var numOtherACS = numACS - numApprovedACS - numDeniedACS;
      if (numOtherACS > 1) {
        //Kick the user out when forms are waiting for approval.
        logger.info("More than 1 ACS form exists.");
        outputCollection[0] =
          "More than one Request for Additional Services forms exist that are waiting for approval.  Research needs to be completed to deny all but one request.";
        outputCollection.push();
        response.json(200, outputCollection);
        return false;
      }

      poDifference = poLimit - poUsage;
      ...
      //Populate returned data and update provider form.
      var formUpdateProviderData = {}; //Loading data to be changed for the form revision.
      formUpdateProviderData["OPG Notified"] = false;
      ...
      Q.allSettled([
        vvClient.forms.postFormRevision(
          formParams,
          formUpdateProviderData,
          formIDProviderForm,
          whoIsProvRev
        ), //Nodejs helper function to post new revision of provider form.
      ]).then(function (promises) {
        var promiseProvUpdt = promises[0];
        if (promiseProvUpdt.state == "fulfilled") {
          var provRetData = promiseProvUpdt.value;
          var formUpdateData = {};
          formUpdateData.provid = whoIsProvider;
          formUpdateData.provname = whoIsProvName;
          formUpdateData.requeststatus = "Approved";
          if (provRetData.meta.status == "201") {
            //Status of 201 indicates clean execution.
            logger.info("Provider Form Revised");
            outputCollection[0] =
              "This request has been updated successfully and set to Approved.";
            ...
            logger.info("Provider form not Revised");
            outputCollection[0] =
              "The form has passed all requirements to approve it.  The provider was not updated successfully to clear the PO Notification checkbox.";
            outputCollection[1] = formUpdateData; //Use this object to return the information for form fillin.
            outputCollection[2] = 1; //Tell the client side that it can save.
            outputCollection.push();
            response.json(200, outputCollection);
