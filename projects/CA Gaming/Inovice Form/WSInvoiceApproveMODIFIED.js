...
module.exports.main = async function (ffCollection, vvClient, response) {
  /*Script Name:   InvoiceApprove
      ...
      05/25/2022 - Renato Corbellini: Defined variables needed for execution groupScreeningBillable and supervisorBillable
  ...
  var basePOValue = getFeeValue(customQueryRespData, "Base PO Value"); //Keeps track of the base value of any starting PO limit.
  var intakeValue = getFeeValue(customQueryRespData, "Intake Value"); //Keeps track of the amount paid per intake questionnaires form.
  var intreatmentValue = getFeeValue(customQueryRespData, "Intreatment Value"); //Keeps track of the amount paid for intreatment questionnaires.
  var endoftreatValue = getFeeValue(
    customQueryRespData,
  ...
  var groupSessionValue = getFeeValue(
    customQueryRespData,
    "Group Session Value"
  ); //Keeps track of the amount paid for each participant in a group session.
  var groupEOTValue = getFeeValue(customQueryRespData, "Group EOT Value"); //Keeps track of the amount paid for conducting a group eot.
  var supervisorLogValue = getFeeValue(
    customQueryRespData,
    ...
    try {
        ...
        if (
          ...
          for (let i = 0; i < formQuestOnInvData.data.length; i++) {
            ...
            let questionnaireDate =
              formQuestOnInvData.data[i]["questionnaire Date"];
            ...
            if (!intakeValue || !intreatmentValue | !endoftreatValue) {
              throw new Error(
      ...
      if (
        ...
        if (
          ...
          for (let i = 0; i < formGroupScreenIncData.data.length; i++) {
      ...
      if (promiseGroupIntreatInc.state == "fulfilled") {
        ...
        if (
          ...
          for (let i = 0; i < formGroupIntreatIncData.data.length; i++) {
            let loadInvoiceArray = {}; //used to load the array.
            loadInvoiceArray.FormType = "Group"; //Load Formtype part of the object.  Group is for this type of form.
            loadInvoiceArray.clienttype = "NA"; //Loading the client type.
            loadInvoiceArray.visitdate =
      ...
      if (supervisorBillable == 1) {
        if (promiseSupervisorLog.state == "fulfilled") {
          ...
          if (
            ...
            for (let i = 0; i < formSupervisorIncData.data.length; i++) {
      ...
      for (let a = 0; a < invRecordControl; a++) {
        newInvoiceLevel =
