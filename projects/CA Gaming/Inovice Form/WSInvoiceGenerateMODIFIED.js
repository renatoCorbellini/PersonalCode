...
module.exports.main = async function (ffCollection, vvClient, response) {
  /*Script Name:   InvoiceGenerate
        ...
        05/12/2022 - Renato Corbellini: Added helper functions parseRes, checkMetaAndStatus and checkDataPropertyExists (Helper Functions Section) used in the getCustomQueryResultByName to bring the active fees from VV instead of hard coding them. Created helper functions getFeeValue and getActiveFeesForQuestDate to get the fee values from the calls to VV. Assigned the corresponding value for each item included in the invoice.
        */
  ...
  function getFeeValue(feeArray, feeName) {
    try {
      const feeFound = feeArray.find((fee) => fee["fee Name"] == feeName);
  ...
  function getActiveFeesForQuestDate(feeArray, questDate) {
    try {
      let activeFeesArray = feeArray.filter(
        (fee) =>
          fee["effectivity Start Date"] <= questDate &&
          questDate <= fee["effectivity End Date"]
      );
      return activeFeesArray;
    } catch (e) {
      // If an error ocurrs, it's because it the fee is not present in the active fees query
    }
  }
  ...
  let shortDescription = `Get all fee forms`;
  const templateName = `Fee Lookup`;

  const getFormsParams = {
    fields: "Effectivity Start Date, Effectivity End Date, Fee Name, Fee Value",
  };

  const getFeesRes = await vvClient.forms
    .getForms(getFormsParams, templateName)
    .then((res) => parseRes(res))
    .then((res) => checkMetaAndStatus(res, shortDescription))
    .then((res) => checkDataPropertyExists(res, shortDescription));

  let getFeesResData = getFeesRes.data;

  // GET THE FEES THAT ARE ACTIVE FOR THE CURRENT DATE (CURRENT DATE >= EFFECTIVITY START DATE AND CURRENT DATE <= EFECTIVITY END DATE)

  const queryName = "zWebSvc Active Fees";
  shortDescription = "Custom Query trying to get active fees";
  ...
  let customQueryRespData = customQueryResp.data;

  //Following are variables that determine the value for each item included in the invoice.
  let basePOValue = getFeeValue(customQueryRespData, "Base PO Value"); //Keeps track of the base value of any starting PO limit.
  let intakeValue = getFeeValue(customQueryRespData, "Intake Value"); //Keeps track of the amount paid per intake questionnaires form.
  let intreatmentValue = getFeeValue(customQueryRespData, "Intreatment Value"); //Keeps track of the amount paid for intreatment questionnaires.
  let endoftreatValue = getFeeValue(
    customQueryRespData,
    "End of Treatment Value"
  ); //Keeps track of the amount paid for eot type questionnaires.
  let groupScreeningValue = getFeeValue(
    customQueryRespData,
    "Group Screening Value"
  ); //Keeps track of the amount paid for conducting group screening if groupScreeningBillable = 1.
  let groupSessionValue = getFeeValue(
    customQueryRespData,
    "Group Session Value"
  ); //Keeps track of the amount paid for each participant in a group session.
  let groupEOTValue = getFeeValue(customQueryRespData, "Group EOT Value"); //Keeps track of the amount paid for conducting a group eot.
  let supervisorLogValue = getFeeValue(
    customQueryRespData,
    "Supervisor Log Value"
  ); //Keeps track of the amount paid for providing supervisor services.
  ...
  d = new Date(formHolder.value);
  let e = new Date(); //Setup date holder variable.
              ...
              try {
                ...
                if (promiseAddlCovSvc.state == "fulfilled") {
                  ...
                  if (
                    ...
                    if (typeof formACSData.data != "undefined") {
                      ...
                      for (let i = 0; i < formACSData.data.length; i++) {
                        if (
                ...
                if (promiseQuestionnaires.state == "fulfilled") {
                  ...
                  if (
                    ...
                    for (let i = 0; i < formQuestNotInvData.data.length; i++) {
                      ...
                      // Get the fee values for each Questionnaire

                      let questionnaireDate =
                        formQuestNotInvData.data[i]["questionnaire Date"];

                      let activeFeesForCurrentQuest = getActiveFeesForQuestDate(
                        getFeesResData,
                        questionnaireDate
                      );

                      //Following are variables that determine the value for each item included in the invoice.
                      intakeValue = getFeeValue(
                        activeFeesForCurrentQuest,
                        "Intake Value"
                      ); //Keeps track of the amount paid per intake questionnaires form.
                      intreatmentValue = getFeeValue(
                        activeFeesForCurrentQuest,
                        "Intreatment Value"
                      ); //Keeps track of the amount paid for intreatment questionnaires.
                      endoftreatValue = getFeeValue(
                        activeFeesForCurrentQuest,
                        "End of Treatment Value"
                      ); //Keeps track of the amount paid for eot type questionnaires.
                      ...
                      if (
                        !intakeValue ||
                        !intreatmentValue | !endoftreatValue
                      ) {
                        throw new Error(
                          "Warning: One or multiple Fee Values are not defined for the specified Invoice Range. The process of generating an Invoice can not be completed until these Fee Values are defined."
                        );
                      }
                ...
                if (promiseQuestInInvoice.state == "fulfilled") {
                  ...
                  if (
                    ...
                    for (let i = 0; i < formQuestOnInvData.data.length; i++) {
                      /* Need the following to build an array of information to calculate billing.  Array held in invoiceRecordset.
                                                        FormType = Treatment
                                                        VisitType = Intake, InTreatment, EOT
                                                        ClientType = Patient, Affected Individual
                                                        ...
                                                        invValue = value of the line item if included in invoice.
                                                        */
                      let loadInvoiceArray = {}; //used to load the array.
                      loadInvoiceArray.instancename =
                ...
                if (promiseClientIntake.state == "fulfilled") {
                  ...
                  if (
                    ...
                    var a = 0;
                    for (let i = 0; i < formIntakeData.data.length; i++) {
                ...
                if (promiseGroupEOTInc.state == "fulfilled") {
                  ...
                  if (
                    ...
                    for (let i = 0; i < formGroupEOTIncData.data.length; i++) {
                      let loadInvoiceArray = {}; //used to load the array.
                      loadInvoiceArray.instancename =
                ...
                if (promiseGroupEOTNot.state == "fulfilled") {
                  ...
                  if (
                    ...
                    for (let i = 0; i < formGroupEOTNotData.data.length; i++) {
                      let loadInvoiceArray = {}; //used to load the array.
                      loadInvoiceArray.instancename =
                ...
                if (invRecordControl > 0) {
                  ...
                  for (let i = 0; i < invRecordControl; i++) {
                    var formTemplateID = "";
                    var formRevID = "";
                    var formRelateDocID = "";
                    ...
                    Q.allSettled([
                      ...
                      if (i == invRecordControl || i == invRecordControl - 1) {
                        if (
