...
module.exports.main = async function (ffCollection, vvClient, response) {
  /*
      Script Name:   LibGroupGetGroupUserInfo
      Customer:      VisualVault
      Purpose:       The purpose of this script is to get a list User IDs, First Name, Last Name and Email addresses for all users who are the member of a group or multiple groups.
      ...
      Psuedo code:
                    1. Get the group ID's for all passed in groups 
      ...
      Date of Dev:   06/27/2022
      Last Rev Date: 06/27/2022
 
      Revision Notes:
      06/27/2022 - Renato Corbellini: script created.
     */

  logger.info("Start of the process LibGroupGetGroupUserInfo at " + Date());
  ...
  function getGroupIDs(groupNames) {
    let groupsToGet = "";

    if (groupNames.value.length > 1) {
      // More than one group, build the quey to include the entire list of groups in an 'IN' clause
      let inClause = "";
      ...
      groupsToGet = inClause;
    } else {
      // Only one group
      groupsToGet = groupsReceived;
    }

    let shortDescription = `Get Groups: ${groupsToGet}`;
    const groupsData = {
      q: `[name] IN ('${groupsToGet}')`,
    };
    ...
    return getGroupsResp.data;
  }

  function getUsersInfo(groupID) {
    try {
      let shortDescription = `Get User Info from Group: ${groupID}`;
      var groupsData = {};
      groupsData.fields =
        "Id, Name, UserId, FirstName, LastName, EmailAddress, Enabled";

      let getGroupUsersResp = vvClient.groups
      ...
      let getGroupsUsersData = getGroupUsersResp.data;

      getGroupsUsersData.forEach((userInfo) => {
  ...
  try {
    ...
    const groupsReceived = ffCollection.getFormFieldByName("groups");
    ...
    if (!groupsReceived) {
      ...
      throw new Error(
        errorLog.join(
          "At least one group is necessary to get the users associated"
        )
      );
    }

    // STEP 1 - Get the group ID's for all passed in groups

    let groupIDs = getGroupIDs(groupsReceived);

    // STEP 2 - For each group found, get user information and load user information into the UserData object.
