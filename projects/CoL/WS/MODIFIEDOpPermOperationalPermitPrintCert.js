...
module.exports.main = async function (ffCollection, vvClient, response) {
  /*Script Name:  OpPermOperationalPermitPrintCert
   ...
   06/20/2022  - Renato Corbellini: Modified query parameters to bring all certificates asociated with the operational Permit ID regardless of the current status. Added helper function findKey used to sort the certificates received from getForms.
  ...
  ***********************/
  ...
  try {
    ...
    *****************/
    ...
    if (getFormsLength === 1) {
      certificateID = getFormsData[0]["instanceName"];
    }
    if (getFormsLength > 1) {
      let sortedCertificates = getFormsData.sort(
        (a, b) =>
          moment(b[findKey(b, "Date Expires")]) -
          moment(a[findKey(a, "Date Expires")])
      );
      // Retunr only the latest certificate
      certificateID = sortedCertificates[0]["instanceName"];
    }
