...
var CallServerSide = function () {
  ...
  formData.push({
    name: "Facility ID",
    value: VV.Form.GetFieldValue("Facility ID"),
  });
  ...
  if (typeof resp.status != "undefined") {
    ...
    if (resp.data[0] != undefined) {
      if (resp.data[0] == "Success") {
        messageData =
          "The License Application has been approved. An inspection will be created.";
        var title = "Success";

        VV.Form.Global.DisplayMessaging(messageData, title);
        VV.Form.SetFieldValue("Status", "Approval", true);
